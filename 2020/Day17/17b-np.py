...
def countaround(x,y,z,w,m):
    c = (np.count_nonzero(m[w-1:w+2, z-1:z+2, y-1:y+2, x-1:x+2]))
    if m[w,z,y,x]: return c-1 #if we've counted ourselves, remove
    else: return c
...
cube = (np.zeros((sizew,sizez,sizey,sizex), dtype=int))
cube[int(len(cube)/2),int(len(cube)/2)] = matrix_2d.copy() #insert starting data
    ...
    for z in range(1,sizez-1):
        for y in range(1,sizey-1):
            for x in range(1,sizex-1):
                for w in range(1,sizez-1):
                    c = countaround(x,y,z,w,cube_old)
                    if c == 3: cube[w,z,y,x] = 1
                    elif c != 2 and cube_old[w,z,y,x] == 1: cube[w,z,y,x] = 0
