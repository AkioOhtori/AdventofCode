...
for line in open("Day07\\data.txt"):
    ...
    hand_sorted = ''.join((sorted(line[0],key=line[0].count,reverse=True))) #ordered hand

    test = line[0].count("J")
    if test != 0:
        for z in range(len(hand_sorted)):
            if hand_sorted[z] != "J":
                line[0] = line[0].replace("J",str(hand_sorted[z]))
                break
    type = list(Counter(line[0]).values())
